...
export default function EditWebStoryPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchStory = async () => {
            try {
                const res = await fetch(`/api/web-stories/${id}`);
                if (res.ok) {
                    const data = await res.json();
                    setTitle(data.title);
                    setSlug(data.slug);
                    setCoverImage(data.coverImage);
                    setLinkedArticleSlug(data.linkedArticleSlug || '');
    ...
    if (loading) return <div className="p-8">Loading story details...</div>;

    return (
        <div className="p-8 max-w-5xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
                <Link href="/admin/web-stories" className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full">
                    <ArrowLeft size={24} />
                </Link>
                <h1 className="text-3xl font-bold">Edit Web Story</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {slides.map((slide, index) => (
                            <div key={index} className="bg-white dark:bg-zinc-800 p-4 rounded-xl shadow border border-gray-200 dark:border-zinc-700 relative group">
                                ...
                                <div className="mb-4">
                                    <label className="block text-xs font-bold mb-2">Slide Image</label>
                                    <ImageUpload
                ...
                <div className="sticky bottom-4 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 p-4 rounded-xl flex justify-end gap-4 shadow-xl z-20">
                    ...
                    >
                        <Save size={18} /> {saving ? 'Saving...' : 'Update Story'}
                    </button>
                </div>
            </form>
        </div>
